...
<xsl:stylesheet
        ...
        <div class="row ds-artifact-item ">
            ...
            <div class="col-sm-3 hidden-xs">
                <xsl:apply-templates select="$metsDoc/mets:METS/mets:fileSec" mode="artifact-preview">
                    <xsl:with-param name="href" select="concat($contextPath, '/handle/', $handle)"/>
                </xsl:apply-templates>
            </div>
                ...
                <xsl:element name="a">
                    <xsl:attribute name="href">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:value-of select="concat($contextPath, '/handle/', $handle)"/>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:attribute>
                    <h4>
    ...
    <xsl:template match="dri:p[@id='aspect.discovery.SimpleSearch.p.hidden-fields']" />
