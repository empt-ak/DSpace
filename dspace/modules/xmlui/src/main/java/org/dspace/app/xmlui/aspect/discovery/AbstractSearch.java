...
public abstract class AbstractSearch extends AbstractDSpaceTransformer implements CacheableProcessingComponent {
    ...
    /**
     * Build the main form that should be the only form that the user interface
     * requires This form will be used for all discovery queries, filters, ....
    ...
    /**
     * Render the given item, all metadata is added to the given list, which
     * metadata will be rendered where depends on the xsl
     *
    ...
    protected void renderItem(org.dspace.app.xmlui.wing.element.List dspaceObjectsList, Item item, DiscoverResult.DSpaceObjectHighlightResult highlightedResults) throws WingException, SQLException {
        ...
        Collection owningCol = item.getOwningCollection();

        if (owningCol != null) {

            String owningColHandle = owningCol.getHandle();
            addMetadataField(itemList.addList(item.getHandle() + ":owningCollection"), owningColHandle);
        }
     ...
     * @param metadataFieldList the wing list we need to add the metadata value
     * to
     ...
     * ("<em></em>") tags, these will be removed & rendered as highlight wing
     * fields.
     *
     ...
     * user.
     * <p/>
     * The search scope when performed by url, i.e. they are at the url
    ...
    protected void buildSearchControls(Division div)
        ...
        /**
         * Add sort by options, the gear will be rendered by a combination of
         * javascript & css
         */
